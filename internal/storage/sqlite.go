...
func NewSQLiteDriver(filePath string) (*SQLiteDriver, error) {
	...
	const (
		...
		defaultCacheTTL  = 300 * time.Second // 300秒
	)
	return NewSQLiteDriverWithCache(filePath, defaultCacheSize, defaultCacheTTL)
}

// NewSQLiteDriverWithoutCache 创建一个新的SQLite驱动，不使用缓存.
func NewSQLiteDriverWithoutCache(filePath string) (*SQLiteDriver, error) {
	// 确保目录存在
